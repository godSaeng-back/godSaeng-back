...
const { Users, Feeds, Likes, Shares, FeedImages, Comment, Sequelize } = require('../models');
...
//     // 각 좋아요한 게시물의 좋아요 수, 댓글 수, 조회 수를 계산합니다.
//     for (let like of likedShares) {
//       const share = like.Share;
...
//       share.setDataValue('commentsCount', commentsCount);
//       share.setDataValue('viewCount', share.viewCount);
//     }
...
router.get('/mypage', checkLogin, async (req, res) => {
  try {
    ...
    const user = await Users.findOne({
      where: { userId },
      attributes: ['userId', 'email', 'profileImage', 'nickname', 'totalPointScore'],
    });
    ...
    // 각 공유글의 좋아요 수, 댓글 수, 조회 수 계산
    for (let share of sharedShares) {
      const likesCount = await Likes.count({ where: { ShareId: share.shareId } });
      const commentsCount = await Comment.count({ where: { ShareId: share.shareId } });
    ...
    // 각 공유글의 좋아요 수, 댓글 수, 조회 수 계산
    for (let like of likedShares) {
      const likesCount = await Likes.count({ where: { ShareId: like.Share.shareId } });
      const commentsCount = await Comment.count({ where: { ShareId: like.Share.shareId } });

      // 추가 정보 세팅
      like.Share.setDataValue('likeCount', likesCount);
      like.Share.setDataValue('commentsCount', commentsCount);
    }
